...
- LXD lab VMs (`listener1-4`, `proxy`, `proxy2`, `retry1-3`, `redis`, `source`, `router1-2`, `metrics`): **stopped**, preserved on disk.
  - Restart with `@/home/light/repo/multicast-test/vm-lab/lab-start.sh`.
  - Re-stop with `@/home/light/repo/multicast-test/vm-lab/lab-stop.sh`.
...
Kubeconfig: `@/home/light/repo/multicast-kube-infra/.kube/k0s.config` (chmod 600).

Node labels applied (`make label-nodes` equivalent — done manually):

```
bsv-mcast/fabric-iface=enp2s0f0
bsv-mcast/node=retry-1
bsv-mcast/role=proxy
bsv-mcast/role-proxy=true
bsv-mcast/role-listener=true
bsv-mcast/role-retry-endpoint=true
```

Multicast sysctls in `/etc/sysctl.d/80-bsv-mcast.conf`:
...
The user-edited copies under `@/home/light/repo/multicast-kube-infra/distributions/k0s/` exist for the documented k0sctl flow but were **not** used for this bring-up — direct `k0s install` is simpler for single-host. Files left in place:
...
`@/home/light/repo/multicast-kube-infra/distributions/k0s/k0sctl.yaml.example:37-41` was repaired for k0sctl v0.20 schema:
...
Applied via `make platform CNI=kube-router NADS=mcast-fabric FABRIC_IFACE=enp2s0f0` from `@/home/light/repo/multicast-kube-infra/`.
...
```
NAMESPACE       NAME           AGE
bsv-mcast   mcast-fabric   ...
```

BGP NADs (`bgp-transit`, `bgp-ibgp`) intentionally skipped — single-node lab, no BGP fabric.

The benign `ClusterSecretStore "bsv-mcast-secret-store"` validation error during `platform-apply.sh` is the unconfigured-provider stub from `@/home/light/repo/multicast-kube-infra/platform/secrets/cluster-secret-store.example.yaml`. ESO works; only the example store is inert. Replace with a real provider when secrets land.

## 6. Apps stage (Phase 5 workloads) — **NOT YET APPLIED**

`make apps ENV=reference-k0s` was **not** run. It would fail because the pod images referenced by the Helm charts (`ghcr.io/lightwebinc/shard-{proxy,listener}`, etc.) are not published yet. See §7.

Chart versions pinned in `@/home/light/repo/multicast-kube-infra/apps/environments/default.yaml:7-11`:
...
GHCR `docker push` to `ghcr.io/lightwebinc/shard-proxy:v0.1.0` returned `403 permission_denied: create_package`. `jefflightweb` is an org Member but lacks rights to create new packages in `lightwebinc`. User opted to migrate to a new GitHub org rather than grant per-package perms.
...
1. **All 8 repos transferred to the new org:**
   - Component code: `shard-proxy`, `shard-listener`, `retry-endpoint`, `subtx-generator`
   - Helm charts: `shard-proxy-helm`, `shard-listener-helm`, `retry-endpoint-helm`, `subtx-generator-helm`
   - (Also useful but not blockers: `multicast-kube-infra`, `shard-common`, `shard-manifest`, `shard-manifest-helm`, `ingress-infra`, `listener-infra`, `retransmission-infra`, `manifest-infra`)
...
cd /home/light/repo/shard-proxy        && IMAGE=ghcr.io/$NEW_ORG/shard-proxy        TAG=v0.1.0 make ci-publish
cd /home/light/repo/shard-listener     && IMAGE=ghcr.io/$NEW_ORG/shard-listener     TAG=v0.1.0 make ci-publish
cd /home/light/repo/retry-endpoint     && IMAGE=ghcr.io/$NEW_ORG/retry-endpoint     TAG=v0.1.0 make ci-publish
cd /home/light/repo/subtx-generator    && IMAGE=ghcr.io/$NEW_ORG/subtx-generator    TAG=v0.1.0 make ci-publish
```
...
```
# @/home/light/repo/multicast-kube-infra/apps/helmfile.yaml.gotmpl
- chart: oci://ghcr.io/lightwebinc/charts/shard-proxy
+ chart: oci://ghcr.io/<NEW_ORG>/charts/shard-proxy
```
...
```bash
cd /home/light/repo/multicast-kube-infra
make apps ENV=reference-k0s
make verify
```

Expected: proxy, listener, retry-1 Ready (single-node lab — only retry-1 is scheduled because only one node and the listener DaemonSet has nodeSelector `bsv-mcast/role-listener=true`, which is on `lax`). retry-2 and retry-3 will be Pending (no matching nodes) and that is acceptable.

The reference-k0s env (`@/home/light/repo/multicast-kube-infra/apps/environments/reference-k0s.yaml`) pins proxy fabric IPv6 to `fd20::20/64` and listener to `fd20::21/64`. The host fabric is on `fd00:dead:beef:1::/64`. The macvlan NAD on `enp2s0f0` will give pods L2 access; pick an IP plan compatible with what the host advertises. Either:
...
`make apps ENV=reference-k0s` again, then scrape `bsl_frames_received_total` from the listener pod via `kubectl -n bsv-mcast exec ... -- wget -qO- http://127.0.0.1:9100/metrics | grep bsl_frames_received_total`. Confirm non-zero delta.

## 10. Roadmap status to flip (remaining)

In `@/home/light/repo/bsv-multicast/containerization/roadmap.md`:
...
| File | Change |
|---|---|
| `@/home/light/repo/multicast-test/vm-lab/lab-stop.sh` | New — `lxc stop --all` wrapper. |
| `@/home/light/repo/multicast-test/vm-lab/lab-start.sh` | New — idempotent ordered VM start. |
| `@/home/light/repo/multicast-kube-infra/distributions/k0s/k0sctl.yaml.example:37-41` | Removed `configPath`, removed `!!str \|` from `config`, restructured `__INLINE_K0S_CONFIG__` placeholder. Compatible with k0sctl v0.20+. |
| `@/home/light/repo/multicast-kube-infra/distributions/k0s/k0sctl.yaml` | Rendered copy of fixed template (gitignored). |
| `@/home/light/repo/multicast-kube-infra/distributions/k0s/k0s-config.yaml` | Local copy with `provider: kuberouter` (gitignored). |
| `@/home/light/repo/multicast-kube-infra/distributions/k0s/hosts.env` | Local copy for `lax` (gitignored). |
| `/etc/k0s/k0s.yaml` (host) | Active k0s ClusterConfig — IPv4-only kuberouter. |
| `/etc/sysctl.d/80-bsv-mcast.conf` (host) | Multicast sysctls for `enp2s0f0`. |
...
No edits made to: any `*-helm` repo, any component repo source, `multicast-kube-infra/Makefile`, helmfile or platform yaml.
...
8. `make apps ENV=reference-k0s && make verify` in `multicast-kube-infra/`.
9. Smoke test by enabling subtx-gen briefly.
10. Mark Phases 4.5/5/6 complete in `roadmap.md`.

## 13. Roll-back

- Destroy apps + platform: `make teardown` in `multicast-kube-infra/`.
